...
public class MailServiceUtility {
	...
	private boolean sendMail(String toEmailId, String subject, String bodyContent, String type) {
		...
		try {
			Transport.send(mimeMessageConfiguration(session, toEmailId, subject, bodyContent, type));
			return true;
	...
	private MimeMessage mimeMessageConfiguration(Session session, String toEmail, String subject, String body, String type) {
		...
		try {
			...
			mimeMessage.setFrom(new InternetAddress(SENDER_EMAIL_ID, type));
	...
	public void recievedMessage(EmailObject mailObject) {
		...
		if (sendMail(mailObject.getEmail(), mailObject.getSubject(), mailObject.getMessage(), mailObject.getType())) {
			return;
		}
